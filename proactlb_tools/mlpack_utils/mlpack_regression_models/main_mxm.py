...
if __name__ == "__main__":
    # read dataset
    input = './input/mxm/chamtool_logfile_rank_0.csv'
    df = pd.read_csv(input, header=None, delimiter='\t')
    df.columns = ['id', 'size', 'Mhz', 'load', 'nan']
    ...
    X_drop = X_pure.drop(columns=['Mhz'])
    scaler = MinMaxScaler()
    scaler.fit(X_drop)
    X_scale = scaler.transform(X_drop)
    X = pd.DataFrame(X_scale, columns=['size'])
