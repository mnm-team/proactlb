...
make clean
cmake -DCMAKE_INSTALL_PREFIX="~/chameleon_tool_dev/install/chamtool_pred3_mig1_off1_dbg_itac" \
    ...
    -DCMAKE_CXX_FLAGS="-DENABLE_COMM_THREAD=1 -DENABLE_TASK_MIGRATION=0 -DCHAM_REPLICATION_MODE=0 -DCHAM_PREDICTION_MODE=3 -DCHAM_PROACT_MIGRATION=1 -DCHAM_STATS_PER_SYNC_INTERVAL -DOFFLOAD_SEND_TASKS_SEPARATELY=1 -DDEBUG_PROACTIVE_MIGRATION=1 -DCHAMELEON_TRACE_ITAC=1 -DTRACE" \
    cmake .
make install

make clean
cmake -DCMAKE_INSTALL_PREFIX="~/chameleon_tool_dev/install/chamtool_pred3_mig1_off2_dbg_itac" \
    -DCMAKE_C_COMPILER=${C_COMPILER} -DCMAKE_CXX_COMPILER=${CXX_COMPILER} -DCMAKE_Fortran_COMPILER=${FORT_COMPILER} \
    -DCHAMELEON_PRINT_CONFIG_VALUES=1 -DCHAMELEON_STATS_RECORD=1 -DCHAMELEON_STATS_PRINT=1 -DCHAMELEON_TOOL_SUPPORT=1 \
    -DCMAKE_CXX_FLAGS="-DENABLE_COMM_THREAD=1 -DENABLE_TASK_MIGRATION=0 -DCHAM_REPLICATION_MODE=0 -DCHAM_PREDICTION_MODE=3 -DCHAM_PROACT_MIGRATION=1 -DCHAM_STATS_PER_SYNC_INTERVAL -DDEBUG_PROACTIVE_MIGRATION=1 -DCHAMELEON_TRACE_ITAC=1 -DTRACE" \
    cmake .
make install
