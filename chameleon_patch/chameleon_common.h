...
extern std::atomic<double> MIN_LOAD_DIFF_PERCENTAGE_TO_MIGRATION;
...
// for debuging proactive offloading tasks
#if DEBUG_PROACTIVE_MIGRATION == 1
extern std::list<std::string> DBG_GIVEN_PREDICT_VAL_ARR;
#endif
...
#if DEBUG_PROACTIVE_MIGRATION == 1
    tmp = nullptr;
    tmp = std::getenv("DBG_PRELOAD_ARR");
    if (tmp != NULL){
        std::string str1_dbg_loads(tmp);
        DBG_GIVEN_PREDICT_VAL_ARR = split(str1_dbg_loads, ',');
    }
#endif
