...
void RequestManager::submitRequests( double startStamp, int tag, int rank, 
    ...
    if(ierr != MPI_SUCCESS) {
       for(int i = 0; i < n_requests; i++) {
            ...
            fprintf(stderr, "Error %d: %s, requests: %d, type: %d\n", eclass, estring, n_requests, type);
            fflush(stderr);
       }
    }
    delete[] stats;
    assert(ierr==MPI_SUCCESS);
    DBP("[SubmitRequests] Type %s: canFinish after MPI_Testall = %d\n", RequestType_values[type], canFinish);
...
void RequestManager::progressRequests(int is_comm_thread) {
    ...
    // worker threads should grab less than comm thread and insert
    // it back into the queue if requests not done
    int num_req_grab = 1;
    if (is_comm_thread) {
    ...
    for(int i = outcount-1; i >= 0; i--) {
        ...
        // threadsafe decrement and get
        int remain_outstanding = _outstanding_reqs_for_group.decrement(gid);
        ...
        if(remain_outstanding <= 0) {
            ...
            if(type == send || type == sendBack) {
                ...
                // disable tracking for very short meta data messages
                // because it might destroy reliability of statistics
    #if OFFLOAD_DATA_PACKING_TYPE > 0
                if(type != recv)
    #endif
...
inline RequestManager::ThreadLocalRequestInfo* RequestManager::get_request_info_for_thread() {
    ...
    RequestManager::ThreadLocalRequestInfo* val = _thread_request_info[__ch_get_gtid()*8];
